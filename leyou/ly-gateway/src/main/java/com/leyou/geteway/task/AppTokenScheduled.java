...
public class AppTokenScheduled {
    ...
    @Autowired
    private AuthClient authClient; // 远程调用接口：申请token
    ...
    public void autoGetAppToken(){
        while (true){
            try {
                log.info("【网关微服务】定时申请token开始");
                this.token = authClient.authorization(prop.getApp().getId(), prop.getApp().getPassword());
                log.info("【网关微服务】定时申请token结束，并成功申请到token，保存到类的全局变量中。");
                break;// 如果成功申请到token，跳出死循环
            }catch (Exception e){
                e.printStackTrace();
                log.error("【网关微服务】定时申请token失败： {}",e.getMessage());
            }
            // 如果失败了休眠10秒再次去申请
            try {
